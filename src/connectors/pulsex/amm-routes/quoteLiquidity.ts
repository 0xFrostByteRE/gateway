...
import { IPulseXV2PairABI, getPulseXV2RouterAddress } from '../pulsex.contracts';
import { formatTokenAmount, getPulseXPoolInfo } from '../pulsex.utils';

export async function getPulseXAmmLiquidityQuote(
  network: string,
  poolAddress?: string,
  baseToken?: string,
  quoteToken?: string,
  ...
  // Resolve tokens
  const baseTokenObj = await pulsex.getToken(baseToken);
  const quoteTokenObj = await pulsex.getToken(quoteToken);
  ...
  if (!poolAddressToUse) {
    poolAddressToUse = await pulsex.findDefaultPool(baseToken, quoteToken, 'amm');
  ...
  if (existingPool) {
    ...
    const pairContract = new Contract(poolAddressToUse, IPulseXV2PairABI.abi, pulsechain.provider);
...
export const quoteLiquidityRoute: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get<{
    ...
    {
      schema: {
        description: 'Get liquidity quote for a PulseX V2 pool',
    ...
    async (request) => {
      try {
        ...
        const poolInfo = await getPulseXPoolInfo(poolAddress, network, 'amm');
        if (!poolInfo) {
