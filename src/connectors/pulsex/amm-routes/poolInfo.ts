...
import { PulseXAmmGetPoolInfoRequest } from '../schemas';
...
export const poolInfoRoute: FastifyPluginAsync = async (fastify) => {
  fastify.get<{
    ...
    {
      schema: {
        description: 'Get AMM pool information from PulseX V2',
        tags: ['/connector/pulsex'],
        querystring: PulseXAmmGetPoolInfoRequest,
        response: {
          200: PoolInfoSchema,
        },
      },
    ...
    async (request): Promise<PoolInfo> => {
      try {
        ...
        // For PulseX, we need to get the pair contract to extract token addresses
        ...
        const token0 = await pulsex.getToken(token0Address);
        const token1 = await pulsex.getToken(token1Address);
        ...
        // Get V2 pair data
        const v2Pair = await pulsex.getV2Pool(token0.address, token1.address, poolAddress);
        ...
        const pairToken0 = v2Pair.pool.token0;
        const pairToken1 = v2Pair.pool.token1;
        ...
        const baseTokenAmount = formatTokenAmount(v2Pair.pool.reserve0.quotient.toString(), pairToken0.decimals);
        const quoteTokenAmount = formatTokenAmount(v2Pair.pool.reserve1.quotient.toString(), pairToken1.decimals);
        ...
        return {
          ...
          feePct: 0.3, // PulseX V2 fee is fixed at 0.3%
          price: price,
