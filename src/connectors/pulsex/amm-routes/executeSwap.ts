...
import { PulseChainLedger } from '../../../chains/pulsechain/pulsechain-ledger';
import { getPulseChainChainConfig } from '../../../chains/pulsechain/pulsechain.config';
...
import { getPulseXV2RouterAddress, IPulseXV2Router02ABI } from '../pulsex.contracts';
import { formatTokenAmount } from '../pulsex.utils';
import { PulseXAmmExecuteSwapRequest } from '../schemas';

import { getPulseXAmmQuote } from './quoteSwap';
...
export async function executeAmmSwap(
  ...
  // Find pool address
  const poolAddress = await pulsex.findDefaultPool(baseToken, quoteToken, 'amm');
  if (!poolAddress) {
    throw httpErrors.notFound(`No AMM pool found for pair ${baseToken}-${quoteToken}`);
  }

  // Get quote using the shared quote function
  const { quote } = await getPulseXAmmQuote(network, poolAddress, baseToken, quoteToken, amount, side, slippagePct);

  // Check if this is a hardware wallet
  const isHardwareWallet = await pulsechain.isHardwareWallet(walletAddress);

  // Get Router02 contract address
  const routerAddress = getPulseXV2RouterAddress(network);
  ...
  const tokenContract = pulsechain.getContract(quote.inputToken.address, pulsechain.provider);
  ...
  if (currentAllowance.lt(amountNeeded)) {
    ...
    throw httpErrors.badRequest(
      `Insufficient allowance for ${quote.inputToken.symbol}. Please approve at least ${formatTokenAmount(amountNeeded, quote.inputToken.decimals)} ${quote.inputToken.symbol} for the PulseX router (${routerAddress})`,
    );
  }
  ...
  try {
    if (isHardwareWallet) {
      ...
      const nonce = await pulsechain.provider.getTransactionCount(walletAddress, 'latest');

      // Build the swap transaction data
      const iface = new utils.Interface(IPulseXV2Router02ABI.abi);
      let data;
      ...
      const gasOptions = await pulsechain.prepareGasOptions(undefined, AMM_SWAP_GAS_LIMIT);
      ...
      let wallet;
      try {
        wallet = await pulsechain.getWallet(walletAddress);
      } catch (err) {
        logger.error(`Failed to load wallet: ${err.message}`);
        throw httpErrors.internalServerError(`Failed to load wallet: ${err.message}`);
      }

      const routerContract = new Contract(routerAddress, IPulseXV2Router02ABI.abi, wallet);
      ...
      receipt = await pulsechain.handleTransactionExecution(tx);
    }
...
export const executeSwapRoute: FastifyPluginAsync = async (fastify) => {
  fastify.post<{
    ...
    {
      schema: {
        description: 'Execute a swap on PulseX V2 AMM using Router02',
        tags: ['/connector/pulsex'],
        body: PulseXAmmExecuteSwapRequest,
        response: { 200: SwapExecuteResponse },
      },
    },
    async (request) => {
      try {
        const pulsechainConfig = getPulseChainChainConfig();
        const {
          walletAddress = pulsechainConfig.defaultWallet,
          network = pulsechainConfig.defaultNetwork,
          baseToken,
          quoteToken,
          amount,
          side = 'SELL',
          slippagePct,
        } = request.body as typeof PulseXAmmExecuteSwapRequest._type;
