...
async function quoteAmmSwap(
  ...
  try {
    // Get the V2 pair
    const pairResult = await pulsex.getV2Pool(baseToken.address, quoteToken.address, poolAddress);
    if (!pairResult.pool) {
      throw httpErrors.notFound(`Pool not found for ${baseToken.symbol}-${quoteToken.symbol}`);
    }
    const pair = pairResult.pool;
    ...
    // Check for insufficient reserves error from PulseX SDK
...
export async function getPulseXAmmQuote(
  network: string,
  poolAddress: string,
  baseToken: string,
  quoteToken: string,
  ...
  if (!pulsechain.ready()) {
    logger.info('PulseChain instance not ready, initializing...');
    await pulsechain.init();
  }

  // Resolve tokens
  const baseTokenInfo = await pulsex.getToken(baseToken);
  const quoteTokenInfo = await pulsex.getToken(quoteToken);
  ...
  const baseTokenObj = pulsex.getPulseXToken(baseTokenInfo);
  const quoteTokenObj = pulsex.getPulseXToken(quoteTokenInfo);
...
async function formatSwapQuote(
  ...
  try {
    ...
    const { quote, pulsechain } = await getPulseXAmmQuote(
      network,
      poolAddress,
      baseToken,
      quoteToken,
    ...
    const gasPrice = await pulsechain.provider.getGasPrice();
...
export const quoteSwapRoute: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.get<{
    ...
    {
      schema: {
        description: 'Get swap quote for PulseX V2 AMM',
    ...
    async (request) => {
      try {
        ...
        if (poolAddressToUse) {
          ...
          const poolInfo = await getPulseXPoolInfo(poolAddressToUse, networkToUse, 'amm');
          if (!poolInfo) {
          ...
          if (baseToken === poolInfo.baseTokenAddress) {
            ...
            const resolvedToken = await pulsex.getToken(baseToken);
          ...
          poolAddressToUse = await pulsex.findDefaultPool(baseTokenToUse, quoteTokenToUse, 'amm');
