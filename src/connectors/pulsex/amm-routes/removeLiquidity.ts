...
import { getPulseXV2RouterAddress, IPulseXV2Router02ABI, IPulseXV2PairABI } from '../pulsex.contracts';
import { formatTokenAmount, getPulseXPoolInfo } from '../pulsex.utils';
import { PulseXAmmRemoveLiquidityRequest } from '../schemas';
...
export const removeLiquidityRoute: FastifyPluginAsync = async (fastify) => {
  ...
  fastify.post<{
    Body: Static<typeof PulseXAmmRemoveLiquidityRequest>;
    Reply: RemoveLiquidityResponseType;
  }>(
    '/remove-liquidity',
    {
      schema: {
        description: 'Remove liquidity from a PulseX V2 pool',
        tags: ['/connector/pulsex'],
        body: PulseXAmmRemoveLiquidityRequest,
        response: {
    ...
    async (request) => {
      try {
        ...
        const baseTokenObj = await pulsex.getToken(poolInfo.baseTokenAddress);
        const quoteTokenObj = await pulsex.getToken(poolInfo.quoteTokenAddress);
        ...
        const pairContract = new Contract(poolAddress, IPulseXV2PairABI.abi, wallet);
        ...
        const routerAddress = getPulseXV2RouterAddress(networkToUse);
        const router = new Contract(routerAddress, IPulseXV2Router02ABI.abi, wallet);
        ...
        try {
          await checkLPAllowance(pulsechain, wallet, poolAddress, routerAddress, liquidityToRemove);
        } catch (error: any) {
        ...
        const gasOptions = await pulsechain.prepareGasOptions(gasPriceGwei, maxGas || AMM_REMOVE_LIQUIDITY_GAS_LIMIT);
