...
export class PulseChainLedger {
  ...
  public async signTransaction(address: string, transaction: Transaction): Promise<string> {
    ...
    const hardwareWallet = await getHardwareWalletByAddress('pulsechain', address);
  ...
  public async isHardwareWallet(address: string): Promise<boolean> {
    const wallet = await getHardwareWalletByAddress('pulsechain', address);
    return wallet !== null;
  }
