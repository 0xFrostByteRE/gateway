...
import { getPulseChainNetworkConfig, getPulseChainChainConfig } from './pulsechain.config';

// information about an PulseChain token
...
export class PulseChain {
  private static _instances: { [name: string]: PulseChain };
  ...
  // PulseChain doesn't have Etherscan service support
  // No etherscanService property needed
  ...
  private constructor(network: string) {
    const config = getPulseChainNetworkConfig(network);
    ...
    // PulseChain doesn't have Etherscan service support
    // No etherscanService initialization needed
  ...
  public static async getInstance(network: string): Promise<PulseChain> {
    if (!PulseChain._instances) {
      PulseChain._instances = {};
    }
    if (!PulseChain._instances[network]) {
      const instance = new PulseChain(network);
      await instance.init();
      PulseChain._instances[network] = instance;
    }
    return PulseChain._instances[network];
  }

  public static getConnectedInstances(): { [network: string]: PulseChain } {
    return PulseChain._instances;
  }
  ...
  /**
   * Check if the PulseChain instance is ready
   */
  ...
  public async estimateGasPrice(): Promise<number> {
    ...
    const cachedEstimate = PulseChain.lastGasPriceEstimate[this.network];
    if (cachedEstimate && Date.now() - cachedEstimate.timestamp < PulseChain.GAS_PRICE_CACHE_MS) {
    ...
    if (supportsEIP1559) {
      try {
        ...
        // PulseChain doesn't have Etherscan support - use RPC provider directly
        // Skip Etherscan API and go directly to RPC provider
        ...
        PulseChain.lastGasPriceEstimate[this.network] = {
          timestamp: Date.now(),
  ...
  public async prepareGasOptions(gasPrice?: number, gasLimit?: number): Promise<any> {
    ...
    if (supportsEIP1559) {
      ...
      if (
        ...
        Date.now() - cachedEstimate.timestamp < PulseChain.GAS_PRICE_CACHE_MS
      ) {
        gasOptions.type = 2;
      ...
      const newCache = PulseChain.lastGasPriceEstimate[this.network];
  ...
  /**
   * Initialize the PulseChain connector
   */
  public async init(): Promise<void> {
    try {
      ...
      logger.error(`Failed to initialize pulsechain chain: ${e}`);
      throw e;
    }
  }
  ...
  public async loadTokens(): Promise<void> {
    logger.info(`Loading tokens for pulsechain/${this.network} using TokenService`);
    try {
      // Use TokenService to load tokens
      const tokens = await TokenService.getInstance().loadTokenList('pulsechain', this.network);

      // Convert to TokenInfo format with chainId and normalize addresses
      this.tokenList = tokens.map((token) => {
        let normalizedAddress = token.address;
        try {
          // Try to checksum the address, but fall back to lowercase if it fails
          normalizedAddress = getAddress(token.address);
        } catch (error) {
          // If checksum fails, use lowercase address for PulseChain
          logger.debug(`Address ${token.address} checksum failed, using lowercase: ${error.message}`);
          normalizedAddress = token.address.toLowerCase();
        }
  ...
  /**
   * Validate PulseChain address format
  ...
  public static validateAddress(address: string): string {
    try {
      ...
      throw new Error(`Invalid pulsechain address format: ${address}`);
    }
  }
  ...
  public async getWallet(address: string): Promise<Wallet> {
    try {
      ...
      const validatedAddress = PulseChain.validateAddress(address);

      const path = `${walletPath}/pulsechain`;
      ...
      if (error.message.includes('Invalid pulsechain address')) {
  ...
  /**
   * Get the first available pulsechain wallet address
   */
  public static async getFirstWalletAddress(): Promise<string | null> {
    ...
    try {
      ...
      try {
        ...
        return PulseChain.validateAddress(walletAddress);
      } catch (e) {
        logger.warn(`Invalid PulseChain address found in wallet directory: ${walletAddress}`);
        return null;
      }
    } catch (err) {
      return null;
  ...
  public async isHardwareWallet(address: string): Promise<boolean> {
    try {
      return await checkIsHardwareWallet('pulsechain', address);
    } catch (error) {
  ...
  /**
   * Close the PulseChain connector and clean up resources
   */
  public async close() {
    if (this.network in PulseChain._instances) {
      delete PulseChain._instances[this.network];
    }
  }
  ...
  public getWrappedNativeTokenAddress(): string {
    const wrappedInfo = PulseChain.WRAPPED_ADDRESSES[this.network];
    if (!wrappedInfo) {
  ...
  public async wrapNativeToken(wallet: Wallet, amountInWei: BigNumber): Promise<ContractTransaction> {
    ...
    const wrappedContract = new Contract(wrappedAddress, PulseChain.WETH9ABI, wallet);
