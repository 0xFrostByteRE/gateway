...
import { PulseChainPollRequest } from '../schemas';
...
export async function pollEthereumTransaction(
  ...
  try {
    ...
    const currentBlock = await pulsechain.getCurrentBlockNumber();
    let txData = await pulsechain.getTransaction(signature);
    ...
    if (!txData) {
      ...
      while (retryCount < MAX_RETRIES) {
        ...
        txData = await pulsechain.getTransaction(signature);
        if (txData) break;
        retryCount++;
      }
    ...
    if (txData) {
      txReceipt = await pulsechain.getTransactionReceipt(signature);
      if (txReceipt === null) {
        ...
        if (txData.gasPrice) {
          const currentGasPrice = await pulsechain.estimateGasPrice();
        ...
        if (connector) {
          try {
            const connectorInstance: any = await getConnector('pulsechain', network, connector);
    ...
    logger.info(`Poll pulsechain, signature ${signature}, status ${txStatus}.`);
...
export const pollRoute: FastifyPluginAsync = async (fastify) => {
  fastify.post<{
    ...
    {
      schema: {
        description: 'Poll PulseChain transaction status',
        tags: ['/chain/pulsechain'],
        body: PulseChainPollRequest,
        response: {
