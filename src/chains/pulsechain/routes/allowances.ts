...
import { getSpender } from '../../../connectors/pulsex/pulsex.contracts';
import { tokenValueToString } from '../../../services/base';
import { logger } from '../../../services/logger';
import { PulseChain, TokenInfo } from '../pulsechain';
import {
...
export async function getPulseChainAllowances(
  ...
  try {
    ...
    if (!tokens || tokens.length === 0) {
      ...
      const allTokens = pulsechain.storedTokenList;
      tokenInfoMap = {};
      for (const token of allTokens) {
        tokenInfoMap[token.symbol] = token;
      }
    } else {
      tokenInfoMap = await pulsechain.getTokensAsMap(tokens);
    ...
    try {
      ...
      if (spender.includes('/') || spender === 'uniswap') {
        ...
        if (spender === 'uniswap/router') {
          ...
          if (spender.startsWith('pulsex')) spenderAddress = getSpender(network, spender);
    ...
    if (isUniversalRouter) {
      ...
      const permit2Contract = new ethers.Contract(PERMIT2_ADDRESS, permit2AllowanceABI, pulsechain.provider);

      await Promise.all(
        Object.keys(tokenInfoMap).map(async (symbol) => {
          try {
            // First check if token is approved to Permit2
            const tokenContract = pulsechain.getContract(tokenInfoMap[symbol].address, pulsechain.provider);
            const tokenToPermit2Allowance = isHardware
              ? await pulsechain.getERC20AllowanceByAddress(
                  tokenContract,
                  address,
                  PERMIT2_ADDRESS,
                  tokenInfoMap[symbol].decimals,
                )
              : await pulsechain.getERC20Allowance(
                  tokenContract,
                  wallet!,
                  PERMIT2_ADDRESS,
                  tokenInfoMap[symbol].decimals,
      ...
      await Promise.all(
        Object.keys(tokenInfoMap).map(async (symbol) => {
          const contract = pulsechain.getContract(tokenInfoMap[symbol].address, pulsechain.provider);
          approvals[symbol] = tokenValueToString(
            isHardware
              ? await pulsechain.getERC20AllowanceByAddress(
                  contract,
              ...
              : await pulsechain.getERC20Allowance(contract, wallet!, spenderAddress, tokenInfoMap[symbol].decimals),
          );
        }),
      );
    }
...
export const allowancesRoute: FastifyPluginAsync = async (fastify) => {
  fastify.post<{
    ...
    async (request) => {
      ...
      return await getPulseChainAllowances(fastify, network, address, spender, tokens);
    },
  );
};
