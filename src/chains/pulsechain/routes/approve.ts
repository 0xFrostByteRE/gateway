...
export async function approveEthereumToken(
  ...
  const fullToken = await pulsechain.getToken(token);
  if (!fullToken) {
  ...
  try {
    ...
    if (isUniversalRouter && universalRouterAddress) {
      ...
      const tokenContract = pulsechain.getContract(fullToken.address);
    ...
    if (!skipERC20Approval) {
      if (isHardware) {
        ...
        const nonce = await pulsechain.provider.getTransactionCount(address, 'latest');
        ...
        const gasOptions = await pulsechain.prepareGasOptions(undefined, APPROVE_GAS_LIMIT);
        ...
        const receipt = await pulsechain.handleTransactionExecution(txResponse);
        ...
        const contract = pulsechain.getContract(fullToken.address, wallet);

        // Call approve function
        const tx = await pulsechain.approveERC20(contract, wallet, spenderAddress, amountBigNumber);
    ...
    if (isUniversalRouter && universalRouterAddress && !skipPermit2Approval) {
      ...
      if (isHardware) {
        ...
        const permit2Receipt = await pulsechain.handleTransactionExecution(txResponse);
        ...
        const wallet = await pulsechain.getWallet(address);
        ...
        // Wait for confirmation with extended timeout
        const permit2Receipt = await pulsechain.handleTransactionExecution(permit2Tx);
